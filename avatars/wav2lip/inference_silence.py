...
parser.add_argument('--checkpoint_path', type=str, 
					help='Name of saved checkpoint to load weights from', default='../models/wav2lip.pth')
...
def datagen(frames):
	...
	for i, m in enumerate(frames):
		...
		#mel_batch.append(np.zeros((1, 80, 16)))
		...
		if len(img_batch) >= args.wav2lip_batch_size:
			...
			mel_batch = np.zeros((len(img_batch), 80, 16,1))
...
def correction(frame_to_save_, pred):
    for j in range(20):
...
def main():
	...
	for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen, 
											total=int(np.ceil(float(len(full_frames))/batch_size)))):
		if i == 0:
