...
def parse_args():
    ...
    parser.add_argument('--device', type=str, default=None,
                        help="force backend: npu[:idx], cuda[:idx], mps, cpu (default: auto-detect)")
    ...
    parser.add_argument('--llm_api_key', type=str, default=os.getenv('LLM_API_KEY', os.getenv('DASHSCOPE_API_KEY', '')),
                        help="API key for the LLM endpoint (falls back to LLM_API_KEY / DASHSCOPE_API_KEY env vars)")
    parser.add_argument('--llm_system_prompt', type=str,
                        default='你是一个知识助手，尽量以简短、口语化的方式输出',
                        help="system prompt sent with every chat request")
