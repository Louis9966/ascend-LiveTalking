...
### 1.2 Ascend 910B Environment

Use the Huawei Ascend PyTorch container (includes CANN and torch_npu):
...
Inside the container, install Python dependencies only — do not install CUDA PyTorch wheels:
...
# compatible with NumPy 2.x or very recent transformers.
pip install numpy==1.26.4 transformers==4.35.2
```

Start the server with `--device npu`:
